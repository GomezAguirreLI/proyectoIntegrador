...
class Usuarios:
    ...
    def actualizar(primer_nom, segundo_nom, apellido_p, telf, email, contrasena, id):
        try:
            cursor.execute(
                "update usuarios set primer_nombre=%s, segundo_nombre=%s, apellido_paterno=%s, telefono=%s, email=%s, contrasena=%s where id=%s",
    ...
    def eliminar(id):
        try:
            cursor.execute(
                "delete from usuarios where id=%s",  # Corregido: era "operaciones", ahora "usuarios"
                (id,)
            ) 
            conexion.commit() 
            return True  
